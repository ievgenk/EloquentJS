...
console.log(Math.round(average(SCRIPTS.filter(lang => lang.living).map(lang => lang.year))))
console.log(Math.round(average(SCRIPTS.filter(l => !l.living).map(l => l.year))))
...
function flatten(twoDimArr) {
    return twoDimArr.reduce((accumulator, currentArr) => {
...
// Dominant Writing Detection

function dominantWritingDirecton(text) {
    let dominantScript = countBy(text, char => {
        let script = characterScript(char.codePointAt(0));
        return script ? script.name : "none";
    }).filter(({ name }) => name != "none").reduce((curr, next) => curr.count > next.count ? curr : next)
    return SCRIPTS.find((script) => script.name === dominantScript.name).direction
}

console.log(dominantWritingDirecton("Hey, مساء الخير."))